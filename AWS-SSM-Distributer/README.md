...
AWS Systems Manager (SSM) の Distributor パッケージを使用して、EC2 インスタンスに CrowdStrike Falcon センサーを自動インストールする手順を説明します。

Falcon sensorは3rd Partyのパッケージとして既に登録されていますので、この機能を使用することで **既存インスタンス** と **新規作成インスタンス** の両方に自動的にセンサーをインストールできます。


本ページは日本語簡易ガイドです。詳細は、以下の英語版をご確認ください。  
...
1. Falconコンソールの **サポートおよびリソース** > **APIクライアントおよびキー** を開く
...
以下3つのパラメータを **SecureString** タイプで作成します。  
...
1. **AWS Systems Manager** > **ノードツール** > **ディストリビューター** > **サードパーティ** を開く
...
以上で、センサーが自動でインストールされます。Falconコンソールにてセンサーが接続されていることをご確認ください。
